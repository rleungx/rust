...
pub fn make_test_name(config: &Config, testpaths: &TestPaths, revision: Option<&str>) -> test::TestName {
    ...
    test::DynTestName(format!("[{}] {}{}", config.mode, path.display(), revision.map_or("".to_string(), |rev| format!("#{}", rev))))
}

pub fn make_test_closure(config: &Config, testpaths: &TestPaths, revision: Option<&str>) -> test::TestFn {
    let config = config.clone();
    let testpaths = testpaths.clone();
    let rev = revision.map(|r| r.to_string());
    test::DynTestFn(Box::new(move || runtest::run(config, &testpaths, rev)))
}
